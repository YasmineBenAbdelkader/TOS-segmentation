...
1 & avec aug, sans crop & 30 & 27 & 0,7556 & modérée ($\approx$3$\times$ sur l'epoch 10) \\
0 & sans aug + crop & 20 & 17 & 0,7187 & \textbf{extrême ($\approx$12,6h de run)} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Incident opérationnel et correction (14/08/2026)} : le run \emph{fold 1, avec
aug, sans crop} ci-dessus (30 epochs, meilleur epoch 27, Dice moyen 0,7556) est un
second run complet de cette configuration. Le premier run (20 epochs, meilleur epoch
17, Dice moyen 0,7401 --- valeur utilisée dans une version antérieure des analyses
ci-dessous) a eu son fichier de poids \texttt{checkpoint\_fold1\_aug\_nocrop.pt}
écrasé par un relancement accidentel de la même commande, interrompu après 1 seul
epoch avant d'avoir pu régénérer les métriques. Le run a donc été refait intégralement
(30 epochs cette fois) plutôt que de tenter de récupérer l'ancien checkpoint. Les deux
runs sont des ré-entraînements indépendants de la même configuration (nouvelle
initialisation aléatoire à chaque fois), pas strictement comparables epoch par epoch
--- mais comme seul le second dispose désormais de poids valides, c'est lui qui sert
de référence pour cette configuration dans tout le reste de ce chapitre ; les anciennes
valeurs (0,7401, et les Hausdorff-95 associés) sont explicitement corrigées ci-dessous
partout où elles étaient citées.
...
\midrule
CLAV & 0,8069 & \textbf{0,8575} & 0,8529 & 0,8422 \\
MSC  & 0,7063 & 0,7537 & 0,7645 & \textbf{0,7646} \\
VSC  & 0,8808 & 0,8861 & 0,8867 & \textbf{0,8876} \\
ASC  & 0,7909 & \textbf{0,8015} & 0,7750 & 0,7945 \\
K1   & \textbf{0,5198} & 0,4882 & 0,5014 & 0,5000 \\
PB   & 0,7557 & 0,7466 & \textbf{0,7704} & 0,7671 \\
\midrule
Moyenne & 0,7434 & 0,7556 & 0,7585 & \textbf{0,7593} \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
CLAV & 5,74 & \textbf{3,52} & 3,96 & 3,75 \\
MSC  & 5,61 & 3,63 & 3,61 & \textbf{3,68 (proche)} \\
VSC  & 1,84 & 2,30 & 1,80 & \textbf{1,83 (proche)} \\
ASC  & 4,25 & 2,16 (proche) & \textbf{2,15} & 2,42 \\
K1   & \textbf{7,57} & 10,09 & 11,51 & 9,27 \\
PB   & 6,08 & 8,71 & 6,56 & \textbf{5,62} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
    \item \textbf{Effet crop (à aug fixé)} : aug+crop vs aug+nocrop (0,7434 vs
        0,7556, écart -0,0122) --- le crop nuit désormais légèrement en présence
        d'augmentation ; noaug+crop vs noaug+nocrop (0,7585 vs 0,7593, écart 0,0008)
        --- quasi nul, inchangé.
    \item \textbf{Effet augmentation (à crop fixé)} : sans crop, l'aug est
        \textbf{quasi neutre} (0,7556 vs 0,7593, écart -0,0037, négligeable) ; avec
        crop, l'aug nuit plus nettement (0,7434 vs 0,7585, écart -0,0151). L'effet
        n'est donc \textbf{pas} ``net et systématique'' comme une version antérieure
        de ce chapitre le concluait avant correction du run aug+nocrop (\S Runs
        effectués) --- il dépend de la position du crop.
\end{itemize}

\textbf{Correction (14/08/2026, après re-entraînement du run aug+nocrop, \S Runs
effectués)} : la lecture initiale (``l'augmentation nuit indépendamment du crop'')
ne tient plus. L'effet de l'augmentation \textbf{dépend nettement de la position du
crop} : quasi neutre sans crop, négatif avec crop actif --- c'est une
\textbf{interaction} entre les deux facteurs, pas un effet principal indépendant.
Le même renversement partiel s'observe en Hausdorff-95 : \textbf{avec crop},
l'augmentation reste nettement pire pour CLAV, MSC et ASC (comme avant) ;
\textbf{sans crop}, elle est désormais \emph{meilleure} pour ces trois mêmes
structures (CLAV 3,52 vs 3,75 ; MSC 3,63 vs 3,68 ; ASC 2,16 vs 2,42) --- seule VSC
reste légèrement moins bonne avec augmentation, crop actif ou non. K1 reste la seule
structure où aug+crop demeure la meilleure config des 4 (7,57\,px).

\textbf{Convergence partielle avec fold 0, une fois la correction du 14/08/2026
appliquée} (\S\ref{sec:phase1-comparaison-aug}) : sur fold 0, l'augmentation sans
crop améliorait nettement le Hausdorff-95 de CLAV ($-44\%$) et MSC ($-39\%$). Avec le
run corrigé de fold 1, \textbf{le même sens d'effet se retrouve} : aug+nocrop a un
Hausdorff-95 plus bas que noaug+nocrop pour CLAV (3,52 vs 3,75) et MSC (3,63 vs 3,68)
--- la ``contradiction directe'' notée dans une version antérieure de ce chapitre
reposait sur le premier run de fold 1, dont le checkpoint a depuis été
accidentellement écrasé puis refait (\S Runs effectués) ; les anciennes valeurs
(CLAV 4,22 ; MSC 4,44) ne sont plus valides. \textbf{VSC reste l'exception} :
Hausdorff-95 plus élevé avec aug qu'sans, sur les deux folds (fold 1 : 2,30 vs 1,83).
\textbf{Toujours pas de conclusion tranchée sur le bénéfice moyen de l'augmentation
sur le Dice} (\S\ref{sec:phase1-aug-signe}), mais une piste plus solide se dégage
maintenant sur le Hausdorff-95 : l'augmentation semble aider spécifiquement
CLAV/MSC \emph{quand le crop est inactif}, sur les deux folds disponibles --- à
confirmer sur folds 2--4 avant toute généralisation.

\textbf{K1 reste globalement plat sur les 4 configs en Dice} (0,488--0,520,
légèrement plus large qu'estimé avant la correction du run aug+nocrop), confirmant
sa faiblesse structurelle indépendamment de l'augmentation et du crop --- cohérent
avec fold 0. Son Hausdorff-95 varie beaucoup plus (7,57 à 11,51\,px) et,
contrairement aux 5 autres structures, semble profiter de l'augmentation \emph{avec}
crop spécifiquement (meilleur score des 4 configs) --- signal isolé, à confirmer sur
...
\textbf{Une fois la correction du 14/08/2026 appliquée (\S Runs effectués), l'image
change} : sur fold 1 sans crop, l'effet de l'augmentation sur le Dice devient
quasi négligeable (-0,004, contre -0,019 dans une version antérieure de ce chapitre)
--- il ne s'inverse donc plus aussi nettement entre les deux folds dans cette
position du crop. \textbf{Avec crop actif, le signe reste opposé entre les deux
folds} (fold 0 : +0,005 ; fold 1 : -0,015), même si les deux magnitudes restent
petites. \textbf{L'effet du crop, lui, reste petit et globalement cohérent sur les
deux folds} (\S\ref{sec:phase1-fold0-grid}, \S\ref{sec:phase1-fold1-grid}). Avec
seulement 2 folds et des effets de cet ordre de grandeur (souvent $<$0,02 en Dice),
la question du bénéfice \emph{moyen} de l'augmentation sur le Dice reste ouverte ---
seuls des folds supplémentaires (2, 3, 4) permettront de voir une tendance
majoritaire. En revanche sur le Hausdorff-95, une piste plus solide se dégage
maintenant (\S\ref{sec:phase1-fold1-grid}) : l'augmentation semble aider CLAV/MSC
\emph{quand le crop est inactif}, sur les deux folds disponibles --- cette piste,
plus étayée qu'avant, reste néanmoins à confirmer sur folds 2--4 avant toute
généralisation dans une synthèse finale.
