...
\begin{description}[style=nextline]
    ...
    \item[Phase 1 --- Modèle baseline] U-Net 2D simple (volontairement non état de l'art), entraîné
        sur les 20 images annotées par série. Dice/Hausdorff-95/NSD par structure comme référence de
        sanité, pas comme objectif d'optimisation. \textbf{Deux modèles spécialisés} (FLASH-only,
        RSSFP-only, pas de modèle pooled) pour répondre directement à la question clinique FLASH vs
        RSSFP au niveau modèle ; méthodologie (perte, augmentation, absence de recadrage ROI) décidée
        a priori à partir de la littérature plutôt que par ablation empirique --- voir le chapitre
        Méthodologie du présent document pour le détail et les justifications.
...
\begin{itemize}[nosep]
    ...
    \item[\checkmark] Phase 0 --- Préparation des données
    \item[$\blacksquare$] Phase 1 --- Modèle baseline (en cours : 1/6 runs terminés --- FLASH fold 0)
