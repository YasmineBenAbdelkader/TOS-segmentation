...
\subsection{Point ouvert}

Avec seulement 13 patients éligibles, un split fixe laisse très peu de patients en
val/test (2 chacun) --- l'évaluation sur un split aussi petit sera bruitée, une variation
d'un seul patient pouvant faire basculer une métrique de façon importante.
\todonote{Décider avant la Phase 1 : garder ce split fixe (simple, mais peu robuste), ou
passer à une validation croisée \emph{GroupKFold} au niveau patient (plus robuste,
plus coûteuse en temps de calcul --- $k$ entraînements au lieu d'un). Discuter aussi
avec l'encadrant si D et G doivent être traités comme un flip horizontal l'un de
l'autre, ce qui influence à la fois le split et l'augmentation de données en Phase 1.}
...
\todonote{Compléter cette section avec le journal des décisions et résultats au fur et
à mesure de l'avancement effectif de la Phase 0 (au-delà de ce qui est déjà acté
ci-dessus) --- notamment la décision split fixe vs cross-validation et la vérification
manuelle des 6 séries en attente (\texttt{0222} $\times$4, \texttt{0223/FLASH D},
\texttt{0224} $\times$4).}
