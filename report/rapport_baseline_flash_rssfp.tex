...
\title{Baseline de segmentation FLASH vs RSSFP \\
\large Méthodologie littérature-first et comparaison au niveau modèle \\
\large Document de suivi de thèse -- complément à \texttt{main.tex}}
...
\chapter{Champ de ce document}

Ce document remplace, pour la question de la comparaison FLASH vs RSSFP, l'approche
suivie dans \texttt{report/sections/04\_phase1\_baseline.tex} (conservé tel quel dans
le rapport principal comme trace de l'exploration initiale, non répété ici). Deux
différences de fond motivent un document séparé plutôt qu'une révision du chapitre
existant :
...
Deux résultats de l'exploration initiale (chapitre Exploration du rapport principal)
restent valides et informent ce document, indépendamment de la méthodologie
d'entraînement retenue :
...
folds 0/1 du modèle pooled) ne sont \textbf{pas} repris ici -- ils restent disponibles
dans \texttt{04\_phase1\_baseline.tex} pour référence historique, mais ce document
part d'une méthodologie déjà décidée, pas d'une comparaison empirique menant à cette
décision.
